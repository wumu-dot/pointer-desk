...
| **按键** | KEY | PA15 | 上排 A15 |
| **串口调试** | TX/RX | PA9/PA10 | ⚠️ **未接线** — 无 USB 转 TTL 模块 |
...
## A4988 步进电机模块（单路驱动扩展板）

### 模块型号

蓝色底板 + 绿色 A4988 子板（可插拔），42步进电机扩展板（拨码开关版）。

### 引脚布局（3×3 九针排）

```
    1 (EN)   2 (STEP)  3 (DIR)   ← 第1排 标 S (信号)
    4 (拨码)  5 (拨码)  6 (VDD)   ← 第2排 标 V (电压)
    7 (GND)  8 (GND)   9 (GND)   ← 第3排 标 G (功率)
```

| 脚位 | 标字 | 接至 | 说明 |
|:---:|:---:|------|------|
| 1 | EN | **PA2** | 低有效使能，空闲拉高防过热 |
| 2 | STEP | STM32 PA0 | TIM2 CH1 PWM 脉冲 |
| 3 | DIR | STM32 PA1 | GPIO 输出 |
| 4 | 拨码 | — (ON) | MS1 |
| 5 | 拨码 | — (ON) | MS2 |
| 6 | VDD | 适配器 3.3V | 逻辑供电 |
| 7 | GND | — | 未接线 |
| 8 | GND | — | 未接线 |
| 9 | GND | 适配器 GND | 逻辑地 |
| **蓝色端子 VIN** | | 适配器 12V | 电机驱动电源 |
| **蓝色端子 GND** | | 适配器 GND | 驱动地 |
| **1A/1B/2A/2B** | | 42步进电机 | 四线插头直插 |

### 模块配置

| 项目 | 值 |
|------|:---:|
| 拨码 MS1/MS2/MS3 | ON / ON / ON → 1/16 微步 (3200/圈) |
| Vref | 0.8V |
| 模块状态 | ✅ 电机可转 |
...
STM32 ── USB 供电（仅 STM32 自己）
```

> ⚠️ TFT 不能从 STM32 3.3V 取电（USB 限流，TFT 吃电导致掉至 2.5V），改从适配器 3.3V 直供。
...
| 自检期间 | LOW | 电机运转 |
| 自检结束 | HIGH | 释放电机 |
| 正常循环 diff≤10 | HIGH | 到位释放，防止过热 |
| 正常循环 diff>10 | LOW | 需要移动时使能 |

### 调试记录

- 旧模块适配器 1.5A 供电不足，插电机 VIN 掉至 0.35V，换新模块后正常
- Vref 从 0V 调至 0.8V
- 电机接入后 VIN 从 12V 掉到 0.35V → 电源电流不足
- 拔掉电机后 VIN 恢复 12V → 模块、接线正常
- **结论：换 12V 2A+ 适配器**
...
| 3 | P1 | 电机未转 | 12V 1.5A 适配器供电不足，插电机后 VIN 掉至 0.35V | 换 12V 2A 适配器 |
