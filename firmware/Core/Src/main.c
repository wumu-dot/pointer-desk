...
/* Definitions for TaskBG */
osThreadId_t TaskBGHandle;
const osThreadAttr_t TaskBG_attributes = {
  .name = "TaskBG",
  .stack_size = 2048 * 4,
...
/* Definitions for QueueBtnEvents */
osMessageQueueId_t QueueBtnEventsHandle;
const osMessageQueueAttr_t QueueBtnEvents_attributes = {
  .name = "QueueBtnEvents"
};
/* Definitions for QueueRenderCmds */
osMessageQueueId_t QueueRenderCmdsHandle;
const osMessageQueueAttr_t QueueRenderCmds_attributes = {
  .name = "QueueRenderCmds"
};
/* Definitions for QueueMotorTargets */
osMessageQueueId_t QueueMotorTargetsHandle;
const osMessageQueueAttr_t QueueMotorTargets_attributes = {
  .name = "QueueMotorTargets"
};
/* Definitions for EvtMotor */
osEventFlagsId_t EvtMotorHandle;
const osEventFlagsAttr_t EvtMotor_attributes = {
  .name = "EvtMotor"
};
/* USER CODE BEGIN PV */
...
int main(void)
{
  ...
  a4988_init();
  temp_sensor_init();
  /* w25q64 由 fs_mgr 模块延迟检测 (FreeRTOS 启动后) */
  ...
  LOG("Hardware init complete, starting scheduler...\r\n");
  /* USER CODE END 2 */
  ...
  /* Create the queue(s) */
  /* creation of QueueBtnEvents */
  QueueBtnEventsHandle = osMessageQueueNew (8, 8, &QueueBtnEvents_attributes);

  /* creation of QueueRenderCmds */
  QueueRenderCmdsHandle = osMessageQueueNew (16, 16, &QueueRenderCmds_attributes);

  /* creation of QueueMotorTargets */
  QueueMotorTargetsHandle = osMessageQueueNew (4, 12, &QueueMotorTargets_attributes);
  ...
  /* creation of TaskBG */
  TaskBGHandle = osThreadNew(StartTaskBG, NULL, &TaskBG_attributes);
  ...
  /* Create the event(s) */
  /* creation of EvtMotor */
  EvtMotorHandle = osEventFlagsNew(&EvtMotor_attributes);
...
static void MX_RTC_Init(void)
{
  ...
  #define OV_RTC_BKP_MAGIC  0x55AA
  ...
  /* backup check deferred to Check_RTC_BKUP (after HAL_RTC_Init) */
  ...
  */
  ...
  /* HAL_RTC_Init 已启用备份域访问, 检查是否首次上电 */
  if (HAL_RTCEx_BKUPRead(&hrtc, RTC_BKP_DR1) != OV_RTC_BKP_MAGIC) {
      need_set_time = true;
      HAL_RTCEx_BKUPWrite(&hrtc, RTC_BKP_DR1, OV_RTC_BKP_MAGIC);
  }
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_2|GPIO_PIN_3|CS_Pin, GPIO_PIN_SET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOC, DC_Pin|RST_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(CSB12_GPIO_Port, CSB12_Pin, GPIO_PIN_SET);
  ...
  /*Configure GPIO pins : PA1 PA2 PA3 CS_Pin */
  ...
  /*Configure GPIO pin : CSB12_Pin */
  GPIO_InitStruct.Pin = CSB12_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(CSB12_GPIO_Port, &GPIO_InitStruct);

  /* USER CODE BEGIN MX_GPIO_Init_2 */

  /* ---- 四按键: PB6(上) PB7(下) PB8(左) PB9(右/确认) ---- */
...
/* printf 重定向到 USART1 (Keil ARMCC + GCC 通用) */
